...
use crate::{
    actions::ESActionWrapper, algos::ContainerMetric, fn_dag::FnId, sim_env::SimEnv,
    CONTAINER_BASIC_MEM,
};
...
impl ScaleNum for TempScaleNum {
    ...
    fn scale_for_fn(&mut self, env: &SimEnv, fnid: FnId, action: &ESActionWrapper) -> usize {
        ...
        if !scale_sign && cur_container_cnt != 0 {
            ...
            // log::info!("平均mem:{}, 平均cpu使用量:{}", container_avg_mem_util, container_avg_cpu_util);

            // 如果有一个大于80%，则进行扩容
            if container_avg_mem_util > 0.8 || container_avg_cpu_util > 0.8 {
                self.mem_decide_to_up_count += 1;
                desired_container_cnt += 1;
